...
func HandleConnection(conn net.Conn, reader *bufio.Reader, isReplicate bool) {
	...
	for scanner.Scan() {
		text := scanner.Text()
		viewText := strings.ReplaceAll(text, "\r\n", "\\r\\n")
		fmt.Println("text:", viewText) // 可视化/r/n 方便查看
		cmd, err := protocol.AnalysisRedisProtocolToCmd(text)
		if err != nil {
			fmt.Println("Error analyzing redis protocol:", err)
			continue
		}
		// AUTH 命令可以在未认证的情况下执行（用于进行认证）
		cmdUpper := strings.ToUpper(cmd[0])
		if !db.IsNoPass && conn != nil {
			if _, ok := db.GetStore().AuthInfo[conn]; !ok {
				if cmdUpper != "AUTH" {
					utils.ErrorResponse(conn, "NOAUTH Authentication required.")
					continue
				}
			}
		}
		...
		if db.GetStore().IsSubscribe(conn) {
			if !slices.Contains(commands.SubscribeCommands, strings.ToUpper(cmd[0])) {
				utils.ErrorResponse(conn, fmt.Sprintf("ERR Can't execute '%s' only (P|S)SUBSCRIBE / "+
					"(P|S)UNSUBSCRIBE / PING / QUIT / RESET are allowed in this context ", cmd[0]))
				continue
			}
		}
		// 丢弃队列里的命令
		if cmd[0] == "DISCARD" {
			if !db.GetStore().IsInTransaction(conn) {
				utils.ErrorResponse(conn, "ERR DISCARD without MULTI")
				continue
			}
			db.GetStore().ClearConnTransactionInfo(conn)
			if !isReplicate {
		...
		if cmd[0] == "PSYNC" {
			...
			db.GetStore().AddReplicaConn(conn, conn.RemoteAddr().String())
			// 发送空的RDB文件
